...
### 🟩 The green box — which model drew the picture (v1.72.0)

`imageLastCall` (now carrying `model` / `engine`) / `_imgRouteModel` /
`imageEngineDescribe` / `IMG_BADGE_MS` / **`imageAnnounce`**, and the
`#imgEngineBadge` element it creates (this app's `#toast` is a single
bottom-centre strip, so the box is its own element). **`polymathlc/cer` and
...
- **A FIXED GREEN, NEVER `--primary`.** Nova Protocol re-skins the tokens, and a box that changed colour with the page would stop reading as the
  one signal it is.
...
permanent on 2026-09-14. Apply it immediately to every signed-in role without
a release flag, preview banner or rollback controls. Preserve auth cleanup,
subject-specific motion preferences, reduced motion and the screen-only CSS
scope. Run `node tools/interface-studio-tests.mjs` when changing the interface.
