...
export function createGrandLineMathAdapter(env) {
  ...
  function selectQuestions(ctx){const data=state(ctx),bank=env.getBank();return selectGrandLineMathBankRound({bank,level:ctx.level,
      ...
      excludedIds:[...failedImages.keys(),...bank.filter(q=>(!ctx.admin&&env.hasSeen?.(q))||unavailable.get(String(q.id))===questionQualitySignature(q)).map(q=>String(q.id))],
  ...
  async function claimRound(ctx) {
    if (!await env.historyReady()) return [];
    // Conflicting tabs refresh the shared ledger in claimQuestions. Replan
    // from that union rather than recycling a stale three-question queue.
    for (let retry=0;retry<4;retry++) {
      if(env.isCurrent && !env.isCurrent(ctx))return [];
      const questions=selectQuestions(ctx);
      if(questions.length!==3)return [];
      if(await env.claimQuestions(questions))return !env.isCurrent||env.isCurrent(ctx)?questions:[];
    }
    return [];
  }
  return {
    getQuestions:ctx=>!ctx.admin&&env.claimQuestions?claimRound(ctx):selectQuestions(ctx),
