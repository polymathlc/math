...
// An explicit teacher preview, independent of TCG releases and student wallets.
// Student keys stay private; student access fails closed until a later release
// supplies a dedicated, authenticated MCQ grading route.
export function installHadesMathBeta(env) {
  ...
  const allowed = () => env.getUser()?.role === 'admin' && !!env.keysAvailable();
  const identity = () => allowed() && grade ? String(env.getUser().uid) + ':hades-preview:' + grade : '';
  ...
  function questions() {
    ...
    return selectHadesMathBankRound({ bank, level: grade,
      progress: state.progress, profile: {}, uid: identity(), served: state.shown,
  ...
  function start(select, stage) {
    ...
    bridge = installHadesLearningParent({ window: win, subject: 'Math', getFrame: () => frame,
      isAllowed: allowed, isActive: () => !!overlay?.isConnected, getIdentity: identity,
      getQuestions: questions,
      markShown: q => { state.shown[q.id] = Date.now(); saveState(); },
  ...
  function open() {
    if (!allowed()) { env.notify('Hades beta is available to administrators after the question bank and answer keys load.'); return false; }
    ...
    doc.body.append(overlay); select.focus(); return true;
  }
